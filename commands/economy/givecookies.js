...
module.exports = {
  ...
  async execute(message, args, cmd, client, discord, profileData) {
    ...
    if (!args.length) return message.channel.send("You need to mention a player to give them :cookie:");
    const amount = args[1];
    ...
    try {
      ...
      return message.channel.send(`${target} has been given **:cookie: ${amount}**`);
    } catch (err) {
      console.log(err);
    }
  },

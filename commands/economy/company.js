...
module.exports = {
    ...
    async execute(message, args, cmd, client, Discord, profileData, settings) {
      ...
      if(args[0] == "shop"){
        ...
        .addField(`⛏️ Miners — ₪ ${minercost.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',')}`, `With miners you can make money by doing ${settings.prefix}mine`)
        .addField(`👷 Workers — ₪ ${workerscost.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',')}`, `Workings increase your hourly pay (${settings.prefix}hourly)`)

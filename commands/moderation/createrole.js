  ...
  module.exports = {
   ...
   if(!message.guild.me.hasPermission("ADMINISTRATOR")){
    return message.channel.send('I need Admin permissions to create roles')
      ...
      message.channel.send('Provid a role name').then(() =>{})
      ...
      collector.on('end', m =>{
        return message.channel.send(`Your role name has been set to \`${rolename}\`, now react with the colour you want the role to be.`).then(async (msg)=> {
          msg.react(`⚫`)
          msg.react(`🔴`)
          msg.react(`🟠`)
          msg.react(`🟡`)
          ...
          message.channel.send(choosetype).then( m1 =>{
            ...
            m1.react('⬜').then(async() =>{
              client.on('messageReactionAdd', async (reaction, user) => {
                ...
                if (reaction.message.channel.id == channel) {
                  if(roletype !== '0') return m1.delete;
                  if(roletype == '0'){
